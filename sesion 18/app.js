...
async function updateUser() {
    ...
    await fetch(`${urlBase}/${updatedUser.id}`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json"
        },
...
//ahora actualizamos la vista de usuarios.
    //funcion de mostrar
    getUsers();
}
...
function renderUsers(users) {
    users.forEach(user => {
        ...
        btnUpdate.addEventListener("click", (event) => {
            event.preventDefault(); // Asegúrate de agregar los paréntesis para llamar a la función preventDefault()
